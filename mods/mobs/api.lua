...
--[[
  ...
  Performs linear interpolation between two angles (a and b) in degrees,
  handling the shortest path across the -180 to 180 degree boundary.

  Arguments:
  a (number): The starting angle in degrees (-180 to 180).
  b (number): The target angle in degrees (-180 to 180).
  t (number): The interpolation factor, a value between 0 and 1.
              0 returns 'a', 1 returns 'b', and values in between
              return an interpolated angle.

  Returns:
  number: The interpolated angle in degrees, within the range of -180 to 180.
]]

local function lerp(a, b, t)
    -- Normalize angles to be within -PI to PI radians for consistency
    -- This step is important if input angles might sometimes fall outside this range
    -- (e.g., 3PI/2 becomes -PI/2).
    a = a % (2 * math.pi)
    if a > math.pi then a = a - (2 * math.pi) elseif a < -math.pi then a = a + (2 * math.pi) end

    b = b % (2 * math.pi)
    if b > math.pi then b = b - (2 * math.pi) elseif b < -math.pi then b = b + (2 * math.pi) end

    local diff = b - a

    -- Handle the shortest path across the -PI/PI boundary
    ...
    -- Ensure the final result stays within the -PI to PI range
...
--print("=== explosion timer", self.timer)
				...
				if self.timer > 1 then
					...
					or self:custom_attack(self, p) == true then
						...
						if self:line_of_sight(p2, s2) == true then
							...
							if playereffects.has_effect_type(self.attack:get_player_name(), "damage_shield") then
								damage = 0
								playereffects.cancel_effect_type("damage_shield", cancel_all, self.attack:get_player_name())
								minetest.sound_play("shieldbreak", { object = self.attack, max_hear_distance = 25, gain = .4 }, true)
							else
							...
							-- Added for Deadmen, to apply armor reduction

							if self.name == "lottmobs:dead_men" and self.attack:is_player() and not playereffects.has_effect_type(self.attack:get_player_name(), "armor_reduction") then
								playereffects.apply_effect_type("armor_reduction", 60, self.attack)
								minetest.chat_send_player(self.attack:get_player_name(), "*** You are frightened and vulnerable to attacks!")
							end

							-- Mirkwood spiders wrap you in webs!
							if self.name == "lottmobs:spider" and math.random(1,4) < 2 and self.attack:is_player() and not playereffects.has_effect_type(self.attack:get_player_name(), "paralyze") then
								playereffects.apply_effect_type("paralyze", 10, self.attack)
								minetest.chat_send_player(self.attack:get_player_name(), "*** You are greatly slowed by the spider's poison!")
							end
		...
		or (self.attack_type == "dogshoot" and dist > self.reach and
			...
			-- Added by Ben for Saruman


			if self.shoot_interval
			and self.name == "lottmobs:saruman"
			and self.timer > self.shoot_interval then

				-- Controls overall frequency of special vs normal attacks
				local rnd_atk = math.random(20)
				...
				if rnd_atk < 3 then
					...
					-- Teleport around like a crazy wizard!
					-- Short range, 5-10

					local tele_dist = math.random(5) + 5
					local check_pos = {}
					...
					local ypos = p.y + self.collisionbox[2] -- just above floor
					local dir_x = -sin(rnd_yaw) * (self.collisionbox[4] + 0.5)
					local dir_z = cos(rnd_yaw) * (self.collisionbox[4] + 0.5)
					...
					for i = -2, 2 do
						...
						and minetest.get_node({x = check_pos.x, y = check_pos.y + i + 1, z = check_pos.z}).name == "air"
						...
						then
							...
							if teleported == false then
								...
								minetest.sound_play( "saruman_teleport", { object = self.object, max_hear_distance = 30, gain = .6 } )
								self.object:setpos({x = check_pos.x, y = check_pos.y + i + 2, z = check_pos.z})
				...
				if rnd_atk == 3 then
					...
					-- Word of Command
					-- Saruman has a powerful voice :)
					-- Destroys all mobs in small radius (friend and foe)
					-- Causes lethargy (slow & blind) in players

					local p = self.object:get_pos()
					local got_something = 0

					local objs = minetest.get_objects_inside_radius(p, 20)

					for _,obj in ipairs(objs) do

						if not obj:is_player()
						and obj:get_luaentity().name ~= "lottmobs:saruman"
						...
						then
							...
							mob_pos = obj:get_pos()
							effect(mob_pos, 10, "web_black.png")

							obj:punch(self.object, 1.0, {full_punch_interval=1.0, damage_groups={fleshy=200},}, nil)

							got_something = 1

						elseif obj:is_player() then

							playereffects.apply_effect_type("blind", 2, obj)
							playereffects.apply_effect_type("paralyze", 2, obj)

							got_something = 1
						end
					end

					if got_something == 1 then
						minetest.sound_play( "saruman_voice", { object = self.object, max_hear_distance = 30, gain = .2 } )
					end

				end

				if rnd_atk == 4 or rnd_atk == 5 then

					-- Small explosion on target
					-- Similar to Grond
					-- There are some who call him "Tim"?

					default.saruman_explode(p, 1, 2, 20) --code in default/functions
...
function mob_class:on_punch(hitter, tflp, tool_capabilities, dir, damage)
	...
	and tflp >= punch_interval then
		...
		-- Changed because mobs were going crazy knockback distances
	...
	-- Remove player invisibility and healer forms

	playereffects.cancel_effect_type("troll_polymorph", 1, name)
	playereffects.cancel_effect_type("invisible", 1, name)
	playereffects.cancel_effect_type("blaze_polymorph", 1, name)
	playereffects.cancel_effect_type("priest_polymorph", 1, name)
...
--		armor = {immortal = 1, fleshy = self.armor}
	...
	if remove_far and ( self.type == "monster" or self.type == "npc" ) then
...
--print("standing in " .. self.standing_in)
	...
	if self.do_custom then
		...
		-- when false skip going any further
		if self:do_custom(dtime, moveresult) == false then
			return
		end
	end
