...
minetest.register_on_dieplayer(function(player)
	...
	--playereffects.normal_form(player)
...
-- Kingsfoil
-- Resurrects nearby players (only if they have not released)
-- So lore friendly it's scary

minetest.register_craftitem( "lottpotion:kingsfoil", {
	description = minetest.colorize("DarkGreen", "Kingsfoil") ..
		minetest.get_background_escape_sequence("lightgoldenrodyellow"),
	inventory_image = "kingsfoil.png",
	wield_image = "kingsfoil.png",
	groups = {forbidden = 1},
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		local player = nil
		local objs = minetest.get_objects_inside_radius(user:getpos(), 10)
		...
		for n = 1, #objs do
			...
			if objs[n]:is_player() then
				local player = objs[n]
				local playername = objs[n]:get_player_name()
				local hp = player:get_hp()
				
				if hp < 1 then
					
					-- Found dead player who has not respawned
					-- Make them alive again :)
					
					lottpotion.players[playername] = {
						speed = 1,
						jump = 1,
						gravity = 1,
						air = 0,
					...
					num_revived = num_revived + 1
					
					minetest.chat_send_player(username, "*** " .. playername .. " was brought back from the brink of death!")
					minetest.chat_send_player(playername, "*** " .. username .. " used some Kingsfoil to revive you!")
					minetest.sound_play( "kingsfoil", { object = player, max_hear_distance = 15, gain = .6 } )
				end
			end
			
		end
		
		if num_revived ~= 0 then
			itemstack:take_item()
			return itemstack
		else
			minetest.chat_send_player(username, "*** No one nearby needs reviving")
		end
	end,
